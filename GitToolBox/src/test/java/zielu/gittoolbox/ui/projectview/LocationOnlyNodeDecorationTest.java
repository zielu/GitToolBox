...
class LocationOnlyNodeDecorationTest {
  ...
  private zielu.gittoolbox.ui.projectview.NodeDecoration decoration;

  @BeforeEach
  void before() {
    when(repository.getCurrentBranch()).thenReturn(new GitLocalBranch(BRANCH));
    config.setPresenter(StatusPresenters.text);
    decoration = new zielu.gittoolbox.ui.projectview.LocationOnlyNodeDecoration(config, repository, repoInfo);
  }
